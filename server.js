...
app.get("/admin/commands",(req,res)=>{
  ...
  // Never discard a command just because this is the client's first poll.
  // Commands are filtered by room + target, so an initial after=0 request
  // can safely receive the commands that are actually addressed to this client.
  const commands=commandQueue.filter(c=>
    c.id>after &&
    c.room===room &&
    (c.targetId==="*"||sameId(c.targetId,playerId))
  );

  const latestId=commands.length
    ? Math.max(after,...commands.map(c=>c.id))
    : after;
